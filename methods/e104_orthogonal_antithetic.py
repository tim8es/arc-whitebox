"""E104 accounting-complete package implementation of the frozen E100 sampler."""
...
def orthogonal_antithetic_numpy(width: int, total_samples: int, seed: int) -> np.ndarray:
    """Unbilled reference generator for tests/target-free diagnostics only."""
    n = int(width)
...
def orthogonal_antithetic_billed(width: int, total_samples: int, seed: int):
    """Production path: every numerical sampling transform is flopscope-billed."""
    n = int(width)
    total = int(total_samples)
    _validate(n, total)
...
class OrthogonalAntitheticEstimator(BaseEstimator):
    """Frozen N=4096 sampler with complete flopscope numerical billing."""
