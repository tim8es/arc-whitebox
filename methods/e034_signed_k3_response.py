...
def relu_k3_moments(xp, mu, var, k3):
    ...
    response = xp.sum(q * (x > 0.0), axis=0)
    return m1, out_var, out_k3, response, xp.max(xp.abs(gamma1))
...
def run_carrier(xp, weights):
    ...
    for w_raw in weights:
        ...
        mu, var, k3_total, response, layer_skew = relu_k3_moments(
            xp, mu_pre, var_pre, k3_pre
        )
        atoms = atoms_pre * xp.reshape(response, (-1, 1))
        k3_diag = k3_total - cp_marginal(xp, atoms, signs)
        max_skew = xp.maximum(max_skew, layer_skew)
        rows.append(mu)

    return xp.stack(rows, axis=0), max_skew, selected
