...
from methods.e033_diagonal_edgeworth import GH_WEIGHTS, GH_Z, run_carrier
...
def _predict_metered(mlp: MLP):
    ...
    with flops.BudgetContext(
        ...
        pred, max_skew, max_kurt = run_carrier(fnp, list(mlp.weights))
...
def main() -> None:
    row = _load_row0()
    mlp, gt = _prepare(row)
    pred, used, residual, wall, max_skew, max_kurt = _predict_metered(mlp)
    ...
    finite = bool(np.isfinite(pred).all()) and np.isfinite(max_skew) and np.isfinite(max_kurt)

    z = np.asarray(GH_Z, dtype=np.float64)
    q = np.asarray(GH_WEIGHTS, dtype=np.float64)
    rule_error = max(
        abs(float(np.sum(q)) - 1.0),
        abs(float(np.sum(q * z))),
        abs(float(np.sum(q * z * z)) - 1.0),
    )
    method_source = Path("methods/e033_diagonal_edgeworth.py").read_text(encoding="utf-8")
    scope_ok = (
        ...
        and "linear_transport" in method_source
        and "relu_edgeworth_moments" in method_source
    )
    ...
    summary = {
        ...
        "normal_rule_max_moment_error": rule_error,
        "finite": bool(finite),
        "scope_ok": scope_ok,
        "gates": gates,
        "decision": decision,
    }
    print("E033_SUMMARY " + json.dumps(summary, sort_keys=True), flush=True)
