...
def _emit(result: dict[str, object]) -> None:
    print("E043_DIAGNOSTIC_JSON=" + json.dumps(result, sort_keys=True))
...
def main() -> None:
    ...
    try:
        ...
        result["patch_counts_ok"] = all(v == 1 for v in provenance["patch_counts"].values())

        module = types.ModuleType("e043_patched_v25_public_mini0")
        exec(compile(patched, "<e043_patched_v25_public_mini0>", "exec"), module.__dict__)
        ...
        stored = int(estimator._e043_stored_births)
        replayed = int(estimator._e043_replayed_births)
        nonterminal = int(estimator._e043_nonterminal_replays)
        replay_ok = stored == (mlp.depth - 1) and replayed == stored and nonterminal == 0
        ...
        result.update(
            {
                ...
                "wall_s": wall_s,
                "stored_births": stored,
                "replayed_births": replayed,
                "nonterminal_replays": nonterminal,
                "terminal_replay_ok": replay_ok,
