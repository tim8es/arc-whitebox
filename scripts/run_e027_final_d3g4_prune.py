...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="e027_") as td_s:
        td = Path(td_s)
        base_path = td / "v25_base.py"
        cand_path = td / "v25_e027.py"
        base_path.write_text(source, encoding="utf-8")
        cand_path.write_text(candidate_source, encoding="utf-8")
        base = _load_module(base_path, "e027_v25_base")
        cand = _load_module(cand_path, "e027_v25_candidate")
    ...
    print("E027_SUMMARY " + json.dumps(summary, sort_keys=True), flush=True)
    Path("e027_result.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
    if decision != "GO":
        raise SystemExit(2)
