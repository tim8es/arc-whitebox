...
def _load_row0() -> dict:
    path = hf_hub_download(repo_id=DATA_REPO, repo_type="dataset", revision=DATA_REV, filename=DATA_FILE)
...
def _run(module, row: dict, *, metered: bool) -> dict:
    ...
    return {"pred": p, "mse": float(np.mean((p[-1] - gt) ** 2)), "flops": used,
            "residual_s": residual, "wall_s": wall, "finite": bool(np.isfinite(p).all())}
...
def main() -> None:
    ...
    scope_ok = candidate_source.count(_EXTRA) == 1 and candidate_source.replace(_EXTRA, "", 1) == source

    with tempfile.TemporaryDirectory(prefix="e028_") as td_s:
        td = Path(td_s)
        base_path = td / "v25_base.py"
        cand_path = td / "v25_e028.py"
        base_path.write_text(source, encoding="utf-8")
        cand_path.write_text(candidate_source, encoding="utf-8")
        base = _load_module(base_path, "e028_v25_base")
        cand = _load_module(cand_path, "e028_v25_candidate")
        row = _load_row0()
    ...
    det_max_abs = float(np.max(np.abs(candidate["pred"] - candidate_repeat["pred"])))
    ...
    summary = {
        "mlp_id": int(row["mlp_id"]), "baseline_mse": baseline["mse"],
        "candidate_mse": candidate["mse"], "mse_ratio": mse_ratio,
        "max_abs_output_difference": max_abs, "baseline_flops": baseline["flops"],
        "candidate_flops": candidate["flops"], "flop_delta": flop_delta,
        "baseline_residual_s": baseline["residual_s"], "candidate_residual_s": candidate["residual_s"],
        "residual_delta_s": residual_delta, "baseline_wall_s": baseline["wall_s"],
        "candidate_wall_s": candidate["wall_s"], "det_max_abs": det_max_abs,
        "projected_utilization": projected_util, "projected_adjusted": projected_adjusted,
        "scope_ok": scope_ok, "pinned_blob": blob, "gates": gates, "decision": decision,
    }
    print("E028_SUMMARY " + json.dumps(summary, sort_keys=True), flush=True)
    Path("e028_result.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
    if decision != "GO":
        raise SystemExit(2)
