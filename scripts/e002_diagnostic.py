    ...
    *,
    ...
    for ridge_lambda in lambdas:
        ...
        for held_out in range(x.shape[0]):
            ...
            predictions.append(
                _predict_one(x[held_out], mean, scale, coefficients, intercept)
            )
    ...
    mean, scale, coefficients, intercept = _fit_one_ridge(x, y, selected_lambda)
    return RidgeRule(
        feature_mean=mean,
        feature_scale=scale,
...
def run_diagnostic(n_rows: int = 20) -> dict[str, object]:
    ...
    for index in range(n_rows):
        ...
        sampling = np.asarray(sampler.predict(mlp, 2**41)[-1], dtype=np.float64)
