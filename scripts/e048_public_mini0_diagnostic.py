...
from methods.e048_public_estimator import Estimator
...
def _emit(result: dict[str, object]) -> None:
    print("E048_DIAGNOSTIC_JSON=" + json.dumps(result, sort_keys=True))
...
def main() -> None:
    ...
    try:
        ...
        core_utilization = float(estimator._e048_core_projected_utilization)
        adjusted = float(raw_mse * max(0.1, utilization))
        residual_s = float(budget.residual_wall_time_s)
        state_rank_max = int(max(max(ranks) for ranks in estimator._e048_state_ranks))
        shapes_ok = bool(all(
            shapes == ((mlp.width,), (mlp.width, mlp.width), (mlp.width,))
            for shapes in estimator._e048_state_shapes
        ))
        representation_ok = bool(
            len(estimator._e048_state_ranks) == mlp.depth
            and state_rank_max <= 2
            and shapes_ok
        )
        ...
        result.update(
            {
                ...
                "core_projected_utilization": core_utilization,
                ...
                "max_abs_state": float(estimator._e048_max_abs_state),
                ...
                "core_util_gate": core_utilization <= CORE_UTIL_GATE,
