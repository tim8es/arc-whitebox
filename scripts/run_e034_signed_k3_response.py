...
from methods.e034_signed_k3_response import run_carrier
...
def _metered(mlp: MLP):
    ...
    with flops.BudgetContext(
        ...
        pred, max_skew, selected = run_carrier(fnp, list(mlp.weights))
...
def _repeat(mlp: MLP):
    with flops.BudgetContext(
        ...
        pred, _, selected = run_carrier(fnp, list(mlp.weights))
    return np.asarray(pred, dtype=np.float64), [int(x) for x in selected]


def main() -> None:
    row = _load_row0()
    mlp, gt = _prepare(row)
    pred, used, residual, wall, max_skew, selected = _metered(mlp)
    repeat, selected_repeat = _repeat(mlp)

    final_mse = float(np.mean((pred[-1] - gt) ** 2))
    util = used / BUDGET
    adjusted = final_mse * max(0.1, util)
    det_max_abs = float(np.max(np.abs(pred - repeat)))
    finite = bool(np.isfinite(pred).all() and np.isfinite(max_skew))

    method_source = Path("methods/e034_signed_k3_response.py").read_text(encoding="utf-8")
    scope_ok = (
        "atoms = xp.zeros((n, 4)" in method_source
        and "signs = xp.ones(4" in method_source
        and "push_fifo" in method_source
        and "leverage_birth" in method_source
        and "GH_Z = (" in method_source
        and "GH_WEIGHTS = (" in method_source
    ...
    gates = {
        ...
        "finite": finite,
        "deterministic": det_max_abs == 0.0 and selected == selected_repeat,
        "scope": scope_ok,
    }
    ...
    print("E034_SUMMARY " + json.dumps(summary, sort_keys=True), flush=True)
    Path("e034_result.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
    if decision != "GO":
        raise SystemExit(2)
