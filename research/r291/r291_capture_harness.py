...
def _residual_f32le(target: bytes, pred: bytes) -> bytes:
    ...
    for i, ((t,), (p,)) in enumerate(
        ...
        # Inputs are binary32. Binary64 subtraction is exact for a difference of
        # two binary32 values; pack_into performs the single binary32 rounding.
        struct.pack_into("<f", out, i * F32_BYTES, t - p)
    return bytes(out)


def _name_order_sha(names: list[str]) -> str:
    # Exact R224 canonicalization.
...
def _best_effort_error_file(root: Path, message: str) -> None:
    try:
        ...
        # Capture must never become a scorer failure channel.
        pass


class CaptureSession:
    """Append R288 sidecar rows; never returns or mutates a scorer value."""
    ...
    def __init__(
        ...
        session = {
            "schema": PROTOCOL + ".session",
            "provenance": self.provenance,
            "expected_panel_sha256": _sha(self.expected_panel_path.read_bytes()),
            "pinned": {
    ...
    def _capture(self, **kwargs: Any) -> None:
        ...
        flags = {
            "budget_exhausted": bool(kwargs["budget_exhausted"]),
            "time_exhausted": bool(kwargs["time_exhausted"]),
            "residual_wall_time_exhausted": bool(kwargs["residual_wall_time_exhausted"]),
            "combined_budget_exhausted": bool(kwargs["combined_budget_exhausted"]),
        }
        ...
        row = {
            ...
            "flops_used": int(kwargs["flops_used"]),
            "effective_compute": float(kwargs["effective_compute"]),
            "scored_prediction_zeroed": zeroed,
            "payload_offset_bytes": expected_offset,
        }

        with self.tmp_vectors.open("ab") as fh:
            fh.write(payload)
        with self.tmp_rows.open("ab") as fh:
            fh.write(_canonical_json_bytes(row))
        self.count += 1

    def safe_capture(self, **kwargs: Any) -> None:
        """Capture best-effort. Errors disable capture and never escape to scorer."""
        if self.disabled:
            return
        try:
...
def capture_from_env_best_effort() -> CaptureSession | None:
    """Future seam initializer; absent/invalid capture never changes scorer flow."""
...
def _validate_staged_payload(root: Path, rows: list[dict[str, Any]]) -> bytes:
    ...
    if len(raw) != RAW_PAYLOAD_BYTES:
        raise CaptureError(
            f"staged vectors size mismatch: expected {RAW_PAYLOAD_BYTES}, got {len(raw)}"
        )
    if len(rows) != ROWS:
        raise CaptureError(f"staged row count mismatch: expected {ROWS}, got {len(rows)}")
    for idx, row in enumerate(rows):
        if int(row["scorer_mlp_index"]) != idx:
            raise CaptureError(f"staged scorer index mismatch at row {idx}")
...
def seal_staged_capture(root: Path, official_report: Path) -> dict[str, Any]:
    """Post-run seal. It is deliberately outside evaluate_estimator()."""
    root = Path(root)
    if (root / "capture_error.txt").exists():
        raise CaptureError("capture_error.txt exists; refusing to seal invalid capture")
    for final in ("vectors.f32le", "manifest.json", "SHA256SUMS"):
        if (root / final).exists():
            raise CaptureError(f"refusing to overwrite sealed path {final}")
    ...
    if session.get("schema") != PROTOCOL + ".session":
        raise CaptureError("session schema mismatch")

    names = [str(r["mlp_name"]) for r in rows]
    if _name_order_sha(names) != EXPECTED_NAME_ORDER_SHA256:
        raise CaptureError("captured name/order hash mismatch")
    if len({str(r["network_id"]) for r in rows}) != ROWS:
        raise CaptureError("captured network_id values are not unique")
    if any(r["r224_target_all_sha256_expected"] != r["r224_target_all_sha256_observed"] for r in rows):
        raise CaptureError("one or more R224 target fingerprints mismatch")
    ...
    manifest = {
        ...
        "vectors_sha256": _sha(vectors_raw),
        ...
        "metering_boundary": (
            "capture occurs evaluator-side after participant BudgetContext; any future "
            "correction application remains a separate metered predict() computation"
        ),
        "pinned": session["pinned"],
        "provenance": session["provenance"],
        "expected_panel_sha256": session["expected_panel_sha256"],
    ...
    vectors_path = root / "vectors.f32le"
    (root / "vectors.f32le.part").replace(vectors_path)
    manifest_path = root / "manifest.json"
    manifest_path.write_bytes(manifest_raw)

    vectors_sha = _sha(vectors_path.read_bytes())
    ...
    (root / "records.jsonl.part").unlink()
    (root / "session.json.part").unlink()
    return {
        "rows": ROWS,
