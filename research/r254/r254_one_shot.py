...
def run(path:Path,weights:list[np.ndarray],label:str)->dict:
    ...
    try:
        ...
        arr=np.asarray(pred,dtype=np.float64); summary=ctx.summary()
        result.update({"shape":list(arr.shape),"prediction_sha256":sha(np.asarray(pred,dtype="<f4").tobytes(order="C")),
                       "flops_used":int(ctx.flops_used),"budget_summary":summary,
...
def candidate_gate(p:dict,c:dict)->tuple[bool,list[str],dict]:
    ...
    pr=float(p.get("budget_summary",{}).get("residual_wall_time_s",0.0))
    cr=float(c.get("budget_summary",{}).get("residual_wall_time_s",0.0))
    ...
    if cr>1.05*pr+0.005:f.append("residual_wall_time")
    return not f,f,m
