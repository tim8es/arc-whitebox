#!/usr/bin/env python3
"""R300 offline analyzer supplement for a future sealed R291/R288 V25 residual capture.
...
ENERGY_SUPPLEMENT = {
    "status": "PRE_DATA_R300_BINARY32_CONSISTENCY_SUPPLEMENT",
    "formula": {
        "B_k": "math.fsum(r_ik**2 for EVAL i)",
        "C_k": "math.fsum(corrected_residual_ik**2 for EVAL i), where corrected_residual bytes are exactly R298 binary32(target-binary32(pred+d))",
    ...
    "arithmetic": (
        "Use the R298 decoded canonical binary32 baseline residuals for B_k and the exact "
        "R298 corrected-residual path for C_k: pred_corr=binary32(pred+d), then "
        "corrected_residual=binary32(target-pred_corr). Never substitute binary32(r-d). "
        "Sums and ratios use Python binary64 math.fsum. This correction is frozen before "
        "any real capture is inspected."
    ),
}
...
def _coordinate_energy_summary(
    ...
    corrected_residuals: dict[int, list[float]],
    eval_idx: list[int],
) -> dict[str, Any]:
    ...
    _require(all(i in corrected_residuals for i in eval_idx), "missing R298 corrected residual row")
    _require(all(len(corrected_residuals[i]) == WIDTH for i in eval_idx), "corrected residual width mismatch")
    ...
    for k in range(WIDTH):
        ...
        c_k = math.fsum(corrected_residuals[i][k] ** 2 for i in eval_idx)
    ...
    corrected_energy_r298_path = math.fsum(
        math.fsum(x * x for x in corrected_residuals[i]) for i in eval_idx
    )
    _require(math.isfinite(total_b) and math.isfinite(total_c), "non-finite global coordinate energy")
    _require(math.isfinite(corrected_energy_r298_path), "non-finite R298 corrected energy")
    _require(
        math.isclose(total_c, corrected_energy_r298_path, rel_tol=1e-15, abs_tol=0.0),
        "coordinate sum_C inconsistent with R298 corrected-MSE residual path",
    )
    ...
    return {
        ...
        "sum_B": total_b,
        "sum_C": total_c,
        "r298_corrected_residual_energy_total": corrected_energy_r298_path,
        "sum_C_matches_r298_corrected_residual_path": True,
...
def analyze_capture(capture_dir: Path, expected_panel: Path,
    ...
    for i in eval_idx:
        ...
        corrected_residuals[i] = corrected_resid
    ...
    coordinate_energy = _coordinate_energy_summary(residuals, corrected_residuals, eval_idx)
    return {
        "schema": "arc.whitebox.r300.capture_analysis.v1",
...
def main() -> int:
    ...
    try:
        ...
        raise SystemExit(f"R300 analyzer error: {exc}") from exc
