...
SPEC = importlib.util.spec_from_file_location("r300_analyzer", HERE / "r300_capture_analyzer.py")
assert SPEC and SPEC.loader
a = importlib.util.module_from_spec(SPEC)
sys.modules["r300_analyzer"] = a
...
def main() -> int:
    checks = {}
    with tempfile.TemporaryDirectory(prefix="r300-selfcheck-") as td:
        root = Path(td)
        ...
        energy_corrected = [
            [-1.0, -2.0] + [0.0] * (a.WIDTH - 2),
            [-1.0, -2.0] + [0.0] * (a.WIDTH - 2),
        ]
        energy = a._coordinate_energy_summary(
            energy_residuals, {0: energy_corrected[0], 1: energy_corrected[1]}, [0, 1]
        )
        ...
        all_zero = a._coordinate_energy_summary(
            [[0.0] * a.WIDTH, [0.0] * a.WIDTH],
            {0: [0.0] * a.WIDTH, 1: [0.0] * a.WIDTH},
            [0, 1],
        )
        ...
        tiny_d = a._f32(1.0e-8)
        baseline_r = a._f32(1.0 - 1.0)
        old_r_minus_d = a._f32(baseline_r - tiny_d)
        pred_corr = a._f32(1.0 + tiny_d)
        corrected_r = a._f32(1.0 - pred_corr)
        checks["binary32_pred_target_counterexample"] = (
            baseline_r == 0.0
            and old_r_minus_d != 0.0
            and pred_corr == 1.0
            and corrected_r == 0.0
        )
        coord = result["coordinate_residual_energy"]
        corrected_total_from_eval_summary = (
            result["eval_summary"]["corrected_mean_mse"]
            * result["capture_integrity"]["eval_rows"]
            * a.WIDTH
        )
        checks["aggregate_coordinate_C_matches_corrected_mse_path"] = (
            coord["sum_C_matches_r298_corrected_residual_path"] is True
            and math.isclose(
                coord["sum_C"],
                corrected_total_from_eval_summary,
                rel_tol=1e-15,
