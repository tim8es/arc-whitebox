...
\[
z=[1;x;h^1;\ldots;h^{16}]\in\mathbb R^p,
\qquad
p=1+17\cdot1024=\boxed{17,409}.
\]
...
Let \(\mathcal K\subseteq\mathbb S_+^p\).
...
\[
\boxed{E[q(X)1_{K_R}]
=\tau_R\operatorname{tr}(A)+p_R\alpha.}
\]
...
- diagonal \(A=\operatorname{diag}(d)\):
  \[
  E[q1_{K_R}]=\tau_R\sum_i d_i+p_R\alpha;
  \]
- block diagonal \(A=\oplus_s A_s\):
  \[
  E[q1_{K_R}]=\tau_R\sum_s\operatorname{tr}(A_s)+p_R\alpha;
  \]
- fixed-basis low-rank-plus-diagonal
  \(A=D+UC U^\top\):
  \[
  E[q1_{K_R}]
  =\tau_R\left(\operatorname{tr}D+
  \operatorname{tr}(C\,U^\top U)\right)+p_R\alpha.
  \]

The tail treatment from R381 is unchanged. If

\[
0\le f_j(x)\le\Lambda_j\|x\|_2,
\]
...
=E[\|X\|_2\,1_{\{\|X\|_2>R\}}]
=\sqrt2\,
\frac{\Gamma((n+1)/2,R^2/2)}{\Gamma(n/2)}
\]

gives a sound upper-tail addition \(\Lambda_j\rho_R\).
...
q_-(x)
=\frac12c^\top x+
\frac{(c^\top x)^2}{2R\|c\|_2}.
\]

For \(t=c^\top x\in[-a,a]\),

\[
q_-(t)=\frac t2+\frac{t^2}{2a}.
\]

If \(t\in[-a,0]\), \(q_-(t)=t(t+a)/(2a)\le0=f(t)\).  
If \(t\in[0,a]\), \(q_-(t)\le t=f(t)\).

Hence \(q_-\le f\) on \(K_R\), while its quadratic matrix is

\[
A_-=\frac{cc^\top}{2R\|c\|_2}
\]

with

\[
\operatorname{tr}(A_-)=\frac{\|c\|_2}{2R}>0.
\]
...
\[
M_j^\pm=\operatorname{diag}(s),\qquad s\ge0.
\]
...
For \(b\mid1024\), also restrict the input-envelope quadratic matrix to \(1024/b\) blocks of size \(b\). Its exact coefficient count is

\[
d_{\rm env}(b)
=
\frac{1024(b+1)}2+1024+1.
\]

Representative exact counts are:

| \(b\) | envelope coeffs \(d_{\rm env}\) | QC+envelope vars | explicit PSD-block scalar entries | off-block zero equalities |
|---:|---:|---:|---:|---:|
...
\ge
\left\lceil
\frac{1024^2}{\lfloor b^2/4\rfloor}
\right\rceil
\]
...
But dense adjacent layers contain \(K_{1024,1024}\). Its treewidth is 1024, so any exact chordal completion has a clique of at least 1025 vertices. Thus an **exact** chordal reformulation cannot reduce all PSD blocks to a constant \(b\ll1024\). Constant-small blocks are an additional inner approximation, not a free exact decomposition.
...
\[
M=D+UU^\top,
\qquad D=\operatorname{diag}(d),\quad d\ge0,
\qquad U\in\mathbb R^{p\times r}.
\]
...
\qquad D\succeq0\text{ diagonal},\quad s_k\ge0.
\]
...
2p^2r
=
\boxed{620,694,079,488}
\]

scalar FLOPs,

\[
\frac{620,694,079,488}{2^{41}}
=
\boxed{0.2822589883580804}.
\]
...
\[
2048(525,825)=\boxed{1,076,889,600}
\]
...
\left[
\frac1{1024}
\|\widehat\mu_i-y_i\|_2^2
\right]
\max\left(0.1,\frac{F_i}{2^{41}}\right)
<
...
\[
\boxed{
\text{NO\_GO\_NO\_SOUND\_LOW\_COST\_QGME\_CERTIFICATE\_CLOSES\_ALL\_PHASE2\_GATES}
}
\]
...
- a problem-specific low-rank theorem reducing every required PSD slack to a fixed \(r\ll1024\);

and, in the same result, an explicit FlopScope-expressible operation schedule proving \(F\le2^{41}\), memory \(\le8\) GB, wall \(\le120\) s, and residual \(\le0.4\) s.
...
- one report path on the isolated R387 branch.
